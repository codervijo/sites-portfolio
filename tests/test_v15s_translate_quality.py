...
def test_write_pnpm_workspace_yaml_creates_when_absent(tmp_path):
    """Fresh project → write the pre-approved allowlist in pnpm v11
    format (packages: + allowBuilds: dict with explicit true)."""
    written = write_pnpm_workspace_yaml(tmp_path)
    assert written is True
    target = tmp_path / "pnpm-workspace.yaml"
    assert target.exists()
    text = target.read_text()
    # 2026-05-22 PM — pnpm v11.1.3 requires `packages:` declaration
    # plus `allowBuilds:` dict. The old `onlyBuiltDependencies:` list
    # is silently ignored under pnpm v11.
    assert "packages:" in text
    assert "- ." in text
    assert "allowBuilds:" in text
    assert "esbuild: true" in text
...
def test_write_pnpm_workspace_yaml_overwrites_pnpm_stub(tmp_path):
    ...
    placeholder text ("set this to true or false"). Lamill must
    overwrite that with the real allowlist."""
...
def test_write_pnpm_workspace_yaml_overwrites_old_v15s_format(tmp_path):
    """The dropaudit.co case (2026-05-22 PM): operator's site has a
    file from v15.S's original format — `onlyBuiltDependencies:` list,
    no `packages:`, no `allowBuilds:` dict. pnpm v11 ignores that
    format → on next `pnpm install` it injects the placeholder stub
    above our content → site stuck in the broken state.

    Lamill recognizes the v15.S-vintage shape (lamill header +
    absence of allowBuilds:) and overwrites with the pnpm-v11
    compatible format on the next translate run."""
    old = tmp_path / "pnpm-workspace.yaml"
    old.write_text(
    ...
    assert "sharp: true" in text
    # Old format gone:
    assert "onlyBuiltDependencies:" not in text


def test_write_pnpm_workspace_yaml_preserves_operator_customization(tmp_path):
    """If operator has already customized the yaml (any format, lamill
    header absent), don't clobber. Idempotency: lamill only overwrites
    pnpm's placeholder stubs and its own v15.S-vintage output."""
    ...
    assert "canvas: true" in text  # operator's customization survives


def test_write_pnpm_workspace_yaml_preserves_hand_curated_old_format(tmp_path):
    """Edge case: operator's file uses the old `onlyBuiltDependencies:`
    format BUT lacks the lamill-generated comment header. Could be a
    hand-curated v9-vintage file. Don't clobber — the absence of our
    header signals operator ownership."""
    legacy = tmp_path / "pnpm-workspace.yaml"
    legacy.write_text(
        "# My hand-curated config, please leave alone.\n"
        "onlyBuiltDependencies:\n"
        "  - esbuild\n"
        "  - my-private-dep\n"
    )
    ...
    assert "hand-curated" in text  # original content survives
    assert "my-private-dep" in text
