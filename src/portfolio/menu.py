...
MENU_GROUPS: list[tuple[str, list[CmdSpec]]] = [
    ("Manage", [
        CmdSpec("1", "summary", "Portfolio overview (counts, value, expirations)",
                ["summary"]),
        CmdSpec("2", "project status", "Status of one project under sites/",
                ["project", "status"],
                positionals=[("name", "project name (fuzzy-matched)")],
                options=[("--json", "Emit JSON instead of table (y/n)", "n")]),
        CmdSpec("3", "cleanup", "Rebuild data/portfolio.json from registrar CSVs",
                ["cleanup"]),
        CmdSpec("4", "check", "Fetch + classify each domain → check snapshot",
                ["check"],
                options=[("--only", "Scope: 'wip' or 'all'", "wip"),
                         ("--concurrency", "Max parallel HTTP requests", "20")]),
    ]),
    ("Build", [
        CmdSpec("5", "domain suggest", "Validation-mode brainstorm + grid + decide",
                ["domain", "suggest"],
                positionals=[("topic", "product idea / topic in quotes")],
        ...
        CmdSpec("6", "bootstrap", "Scaffold a new sites/<domain>/ project",
                ["bootstrap"],
        ...
        CmdSpec("7", "deploy", "Set up GitHub repo + Cloudflare Pages",
                ["deploy"],
...
def collect_args(cmd: CmdSpec) -> list[str] | None:
    ...
    for label, description in cmd.positionals:
        ...
        prompt = f"  {label}  [dim]({description})[/]"
    ...
    if cmd.options:
        use_defaults = typer.confirm(
            "  Use defaults for the optional flags?",
            default=True,
        )
        if not use_defaults:
            for flag, description, default in cmd.options:
                hint = f" (default: {default})" if default else ""
                prompt = f"    {flag}  [dim]{description}[/]{hint}"
                val = typer.prompt(prompt, default="", show_default=False).strip()
                if not val:
                    continue
