...
# 2026-05-22 PM — no-topic "latest trends" surface was attempted and
# withdrawn. All three approaches failed against Google's current
# API state:
#
#   1. pytrends.trending_searches(pn=...) → 404 (deprecated endpoint)
#   2. pytrends.today_searches(pn=...)    → 404 (also broken)
#   3. trends.google.com .../daily/rss?geo=... → 404 (RSS retired
#      with Google's 2024 trending-searches page redesign)
#
# Google's current trending-searches UI (https://trends.google.com/
# trending) uses an undocumented internal `/_/TrendsUi/data/
# batchexecute` endpoint with a quirky JSON-in-form-encoded RPC
# format. Not feasible to consume from a simple HTTP client.
#
# Decision: latest-trends discovery happens in the Google Trends
# web UI; portfolio CLI sticks to the topic-mode surface (where
# pytrends' build_payload + interest_over_time + related_queries
# still works). See `docs/bugs.md` for the iteration history.
