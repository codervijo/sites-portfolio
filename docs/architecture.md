...
| `indexnow.py` | v30.A | IndexNow key provisioning (submission lands v30.B). `generate_key()` (32-hex), `key_file_path`, `is_provisioned(repo)`, and `provision(repo)` — idempotently generate a key, write `public/<key>.txt`, and upsert `[index]` via `lamill_toml_edit.set_table` (ADR-0018). The key file is self-authenticating + deploys with the site; a ping reaches Bing/Yandex/Naver/Seznam/Yep (not Google). See ADR-0020 + `docs/indexing-module-plan.md` |
