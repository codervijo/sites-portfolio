...
| `gtrends.py` (v19.B + 2026-05-22 PM mitigations + latest-trends) | Google Trends, **two modes with two transports**: (1) topic-specific `fetch_trends(topic, ...)` returns `TrendsPayload` (interest-over-time + related queries) via **pytrends** — that library still works for `build_payload + interest_over_time + related_queries`; (2) no-topic `fetch_latest_trends(region)` returns `LatestTrendsPayload` (daily trending searches) via **direct RSS** at `https://trends.google.com/trends/trendingsearches/daily/rss?geo=...` (httpx + stdlib `xml.etree.ElementTree`). pytrends' `trending_searches` AND `today_searches` both 404 against Google's current API as of 2026-05-22; the public RSS feed is the resilient path. Topic path keeps the `_make_pytrends_client()` UA rotation + lazy `ImportError` → typed `GTrendsError` with `uv sync` hint. Caches: `data/gtrends/<topic-hash>.json` (topic mode, schema `gtrends-v1`) + `data/gtrends/latest_<region-hash>.json` (latest mode, schema `gtrends-latest-v1`); 24h TTL each. **429 mitigations**: `GTrendsRateLimitError` subclass on both paths; `fetch_trends` / `fetch_latest_trends` both fall back to ANY cached payload via `load_any_cached*` helpers (renderer surfaces yellow stale-warning header). L4 SerpAPI fallback NOT shipped — pinned. | `fetch_trends`, `fetch_latest_trends`, `load_cached`, `load_any_cached`, `load_cached_latest`, `load_any_cached_latest`, `save_cached`, `save_cached_latest`, `payload_age_hours`, `latest_payload_age_hours`, `TrendsPayload`, `LatestTrendsPayload`, `GTrendsError`, `GTrendsRateLimitError` |
