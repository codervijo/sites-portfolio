...
- **Fixed** — `_last_crawl_by_domain()` (`check_render.py`) now reads **both** cache schemas: the newer `v16c_inspections[].last_crawl_time` and the older `coverage[].last_crawl_at` (extracted into a shared `_home_crawl_from_items(items, field, domain)` helper that prefers the apex-homepage entry). Every cached domain now has a fallback floor, so when `probe_gsc_last_crawl`'s live call transiently returns None the column shows the cached date instead of "—". Verified against the real cache: the fallback map grew 33 → 39 domains, hybridautopart floors to `2026-06-04`, newer-schema domains unchanged (threadradar `2026-07-03`). Tests: `test_home_crawl_from_items_picks_homepage_across_schemas` + `test_last_crawl_by_domain_covers_legacy_coverage_schema`; 483 render/seo/crawl tests green. Residual (accepted): when the live call blinks None the value can still *differ* run-to-run (fresh live date vs older cached floor), but it never blanks to a false "never crawled" — the documented Expected. The deeper fix (persist live crawl dates so the floor advances) is out of scope; would require writing cache during `fleet seo --refresh`.
- **Severity** — minor (cosmetic-ish but misleading — "—" reads as "never crawled" on a site Google crawls regularly).
