...
- **Remaining (separate, not Astro)** — **vite-react-ssg** sites (e.g. isitholiday.today) keep a bare `index.html` shell with title + JSON-LD in a React `<Seo>` component — 070/078/079 false-fail there, a *different* stack; out of scope, same bug class.
