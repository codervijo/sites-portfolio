...
- **Remaining (separate, not Astro)** — `CHECK_079` json-ld via `set:html={JSON.stringify(obj)}` isn't statically evaluable (JS object w/ `.map()`), still unresolved. And **vite-react-ssg** sites (e.g. isitholiday.today) keep a bare `index.html` shell with the title in a React `<Seo>` component — 070 false-fails there too, a *different* stack from Astro; out of scope for this fix, same bug class.
