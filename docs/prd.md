...
### v27 — per-site todo + stack tracking in `lamill.toml` *(new 2026-05-28; scope expanded 2026-05-29 to absorb `[stack]` alongside `[[todo]]` — one durability pass for both additive tables; v27.A shipped 2026-05-29 — decisions locked + ADR-0017 + `[[todo]]` and `[stack]` shapes in `docs/CLAUDE.md § Locked target shapes`)*
