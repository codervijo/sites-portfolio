...
Strict sequence (option a). 21 phases total; v1.A through v1.F shipped — all of v1 is done.
...
| **v3.A** | Bootstrap — scaffold a new project *(Power 2 — ship-ready scaffold)* | `portfolio bootstrap <domain> [--stack=astro\|vite\|...] [--with-ingester]` · creates `sites/<domain>/` · `git init` + initial commit · stack scaffold via the central builder's `make new` (Astro default for content/SEO sites; Vite for app-style; flag overrides) · standard `/project-init` files (AI_AGENTS.md with Building/Deployment sections, docs/prd.md, docs/Prompts.md, README, .gitignore) · `--with-ingester` adds a `scripts/` dir with an ingester template for projects that scrape data and feed the app · filesystem-only at this phase; no network |
