...
Note on v9 insertion + tier shift (2026-05-17 PM):

- Operator-direction: bootstrap UX gaps surfaced when `agesdk.dev`'s
  scaffold ran — `new bootstrap` writes template-only content for
  `AI_AGENTS.md` / `docs/growth.md` (no operator input prompted), and
  doesn't register the new domain in `portfolio.json` (had to be
  manually appended). These improvements are highest-priority and
  pre-empt the v9 deploy-declarations tier.
- New **v9** tier added: bootstrap UX — canonical AI_AGENTS sections
  + interactive prompts + portfolio.json auto-update + growth.md
  seed. Five shippable letters (v9.A–v9.E); see rows above. v9.E
  added 2026-05-17 PM as the consolidation step: refactor v9.A's
  in-code canonical-sections list to a JSON-driven single source
  of truth so the schema is editable as data, not Python code.
- Everything from old v9 onward shifts right by one major tier:
  - **v9 → v10** (per-site deploy declarations / lamill.toml). Scope
    expanded to include a `[backend]` section for non-JS-rendering
    server stacks (DB + server framework + backend-hosting target).
  - **v10 → v11** (fleet hosting view).
  - **v11 → v12** (analytical roll-ups).
  - **v12 → v13** (deploy verification).
- AI_AGENTS canonical schema is 10 sections (not 9): added **ICP**
...
| **v8.I** ✅ | Wire primary pass into `new research` orchestrator | First user-visible v8.E-series feature. Calls `run_primary_pass` from `new_research` after the mechanical gates step; renders an "Interpretive verdict (Claude):" section in the human output (default-on, no flag — ~5-15s per run, $0 incremental cost via Claude CLI subscription); persists `primary_verdict` + `rendered_prompt` + `prompt_version` + `model_id` + `interpretive_cost_usd` + `interpretive_duration_s` in the cluster snapshot. Snapshot schema bumped to v2.1. Shipped `9711749`. |
| **v8.J** ✅ | Adversarial audit payload builder | `audit_pass.build_audit_payload(cluster, *, primary_verdict, operator_profile)` — extends the v8.E primary payload with `primary_response_markdown` reconstructed from the persisted parsed verdict. `_reconstruct_primary_markdown` helper strips `blind_spot_self_report` by default (anti-anchoring per the audit prompt's instruction). Shipped `d284ecd`. The remaining audit-pass arc (prompt rendering, response parser, runner, reconciliation, CLI wiring, polish, docs) was promoted to a dedicated **v12** tier on 2026-05-17 PM — see notes in §5 and rows v12.A–v12.G below. |
| **v9.A** ✅ | Canonical AI_AGENTS.md section schema + conformance check + tier-1 fix *(new 2026-05-17; shipped `a5fcdac`)* | Lock the 10-section AI_AGENTS canonical schema: `## Summary` / `## Audience` / `## ICP` / `## Goals` / `## Tech stack` / `## Building info` / `## Deployment info` / `## Content strategy` / `## Versioning` / `## Conventions` (4 operator-input — Summary/Audience/ICP/Goals/Content strategy — and 5 template-driven — Tech stack/Building info/Deployment info/Versioning/Conventions). New conformance check `ai-agents-md-has-canonical-sections` verifies all 10 H2 headings exist; tier-1 fix injects missing sections with `(to be filled in)` placeholders so `project check <domain>` lights red until populated. Foundation for v9.B — interactive prompts have a concrete write target. ~3-4h. |
| **v9.B** ✅ | `new bootstrap` interactive prompts for operator-input AI_AGENTS sections *(new 2026-05-17; shipped `741fe74`)* | After the v9.A schema lands, `lamill new bootstrap <domain>` prompts the operator for the 5 operator-input sections (Summary / Audience / ICP / Goals / Content strategy). Defaults to `(to be filled in)` placeholder when blank; `--non-interactive` skips all prompts; per-section flags (`--summary "X"`, `--audience "Y"`, `--icp "Z"`, `--goal "W"`, `--content-strategy "V"`) let scripts pre-populate. Operator-input is merged with the template-driven sections at scaffold time. ~3-4h. |
| **v9.C** ✅ | `new bootstrap` domain-registration prompt + portfolio.json auto-update *(new 2026-05-17; shipped `530b988`)* | Bootstrap asks "Is `<domain>` registered? [Y/n]" + registrar (`porkbun` / `godaddy` / `namecheap` / `other`) and auto-appends the row to `data/portfolio.json` with conservative placeholders (matches the manual edit done for agesdk.dev on 2026-05-17 — `category=Under build`, `status=Active`, dates = today / +1y, renewal_price/nameservers null for the next refresh to fill). When `[n]`, status = `Pending` and the inventory row reminds the operator to come back. Closes the "new domain on disk but not in portfolio.json" gap that bit agesdk.dev. ~2-3h. |
| **v9.D** ✅ | `new bootstrap` growth-hypothesis prompt → seeded `docs/growth.md` *(new 2026-05-17; shipped `6704987`)* | Bootstrap prompts for the initial growth hypothesis (one paragraph: "what's your bet for how this site reaches its audience?") and writes it as the first dated H2 entry in `docs/growth.md`. Replaces the empty `docs/growth.md` stub that today's bootstrap creates — operator now has a real first entry to react to rather than a blank doc. `--non-interactive` / `--growth-hypothesis "X"` flags for scripted use. ~1-2h. |
| **v9.E** ✅ | Canonical-sections TOML-driven single source of truth *(new 2026-05-17; shipped `37f9398` as TOML, not JSON)* | Refactor v9.A's in-code canonical-sections list to a single JSON file (probably `data/canonical_sections.json` or similar; final path TBD). A small loader module reads the JSON at runtime; v9.A's conformance check, v9.B's interactive prompts, and the bootstrap template renderer all consume from the loader. **Operator value**: the canonical schema can be edited by hand (rename a section, reorder, change source=operator vs source=template, tweak placeholder text) without touching Python code, restarting a process, or shipping a release. No behavior change vs v9.A — v9.E is a pure consolidation that locks the "one place" guarantee. ~1-2h. |
| **v10.A** | `lamill.toml` per-site deploy declaration *(planned 2026-05-14, full PRD inlined in §8.3; renumbered 2026-05-17 — was v9.A; scope expanded 2026-05-17 to include `[backend]` section)* | Visible TOML file at each `sites/<domain>/` repo root declaring where the site deploys. Closes the gap for hosts without canonical configs (HostGator, WordPress, custom VPS). Schema: `[deploy]` (platform, account, branch, custom_domains) + `[hosting]` (cPanel/FTP breadcrumbs when applicable) + `[backend]` (DB + server framework + backend-hosting target for non-JS-rendering server stacks — postgres/sqlite/duckdb/redis/none × go-fiber/fastapi/express/node-bare/rust-axum/none × fly.io/managed-provider/none) + `[notes]`. CLI: `project set-deploy`, `project show-deploy`, `fleet repos --add-deploy-declarations` migration. `new bootstrap` writes it automatically. ~12-16h. 8 open questions in PRD. |
| **v10.B** | Drift detection + lamill.toml conformance checks *(deferred from v10.A; full PRD TBD; renumbered 2026-05-17 — was v9.B)* | CHECK_xxx series comparing `lamill.toml` declaration vs DNS-resolved actual + live HTTP probe (extends `project diagnose`'s existing inference). `has-lamill-toml` + `lamill-toml-valid` + `deploy-drift` checks. ~6-8h. |
...
| **v12.A** ✅ | Adversarial audit prompt rendering *(new 2026-05-17 PM — moved out of v8.K)* | `audit_pass.render_audit_prompt(payload)` (commit `1ecfebf`) — loads `prompts/adversarial_audit_v1.md`, appends the audit payload JSON (already built by v8.J) in a fenced block. No operator-var placeholders in the audit prompt (operator context flows through the payload's `operator_profile_summary` field); the renderer runs `render_prompt()` anyway as drift protection — if a future prompt edit adds a `{{var}}`, the renderer fails loud at the prompt-assembly boundary rather than shipping `{{var}}` text to the LLM. Parallel to v8.F's `render_primary_prompt`. 12 tests. |
...
### 8.3 — v10.A · lamill.toml per-site deploy declaration *(renumbered 2026-05-17 — was v9.A)*
...
### 8.4 — v11.A · `fleet hosting` — fleet-wide Vercel/CF deploy state *(renumbered 2026-05-17 — was v10.A)*
