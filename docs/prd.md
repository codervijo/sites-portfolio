...
### v28 — topic-aware TLD expansion in `new domain` *(new 2026-06-01 after a family-archive topic surfaced that the generic `.com/.app/.xyz` ladder misses the topically-perfect TLDs — `.family`, `.fm`, `.gift`, `.photos`; v28.A/B shipped 2026-06-01 — decisions locked + `THEME_MAP`/`TOPICAL_TLDS`/`topical_tlds_for` + `lookup_renewal`, 10 tests; v28.C shipped 2026-06-01 — topic→theme selection folded into the vocab call + in-place topical-column merge (`merge_topical=not tlds`), 14 tests)*
