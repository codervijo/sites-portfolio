...
| **v4.A** | Mark / unmark shortlist + grid alphabetical sort *(headline; new 2026-05-08; deferred from v3.E menu stub item 6)* | Two coupled changes that ship together. **(1) Grid sort flips from score-desc to alphabetical-by-name** (`row.name` ascending). Score ranking was right for "show me the best candidate first" but this phase reorients the grid for "build a shortlist by scanning names" — you find a candidate by leading letters, not by ranking. Anchors / Pick / Why columns are unchanged; only the row order. **(2) Mark / unmark shortlist.** Adds menu item **6. Mark / unmark for shortlist** (and item 7 placeholder hint pointing at v4.B). Picker accepts `m N` / `m <name>` to mark a candidate as a finalist; `u N` / `u <name>` to unmark; `p` to print the current shortlist with per-candidate cell summary. Shortlist persists across menu iterations until quit. Shortlist count appears in the menu label when nonzero (e.g. `6. Mark / unmark for shortlist (3 marked)`). User-added names are markable just like LLM-generated ones. Implementation: shortlist is a list of names (not row references) so it survives grid mutations from add-names / future widen. Tests cover mark/unmark/list/count-display + the alphabetical sort assertion. |
