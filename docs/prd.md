...
| v27.J | ☐ | **(Bug-driven) Migrate legacy rewrite paths to upsert.** `settings deploy set` / deploy pipeline (×2) / `hosting` still full-rewrite via the serializer and would clobber `[content]`/comments on a content-bearing site. Tracked in `docs/bugs.md`; graduate to this phase if it bites. |
