...
| v37.G | planned | **One-strike circuit-breaker (backstop).** Arm on first quota draw; trip on the first checkpoint with quota-spent + build-failing + flat-diff → kill + hand off/report. Demoted to backstop by the diagnosis (v37.E prevents the failure mode upfront); operator picked the one-strike policy 2026-06-14. |
