...
| v41.B | planned | **Deploy-health check — catch builds that never shipped.** Extend `CHECK_144 has-version-stamp` (the intended freshness primitive, per the post-mortem) to compare the *deployed* commit against repo `HEAD` and fail when they diverge (= latest build failed / never deployed). Also fix the version-stamp build-artifact convention so the stamp lands in the *public* output (it currently writes to `dist/server` on the TanStack/Worker stack, never served, so `/version.json` returns SPA HTML). Covers both cf-pages + cf-workers via the version-stamp (and/or the CF deployments API, which the token *can* read — the Workers Builds CI API is 403). |
