...
Strict sequence (option a). 25 phases total; v1.A–v1.F + v2.A + v2.B + v3.A + v3.B + v3.C + v3.D + v3.E shipped (13/25). **v3 complete.** v4.A (validation-suggest extensions: finalists, ask AI, widen) queued next.

Note on read/write surfaces: portfolio is **read-only** through v2 (domain suggest). **v3 (bootstrap) is the first write surface** — it creates new project dirs, runs `git init`, scaffolds files. **v4.F (remediation) is the second write surface** — it modifies existing project dirs to fix conformance gaps. Everything else (v4.A–E, v5, v6) is read-only.
...
| **v4.A** | Validation-suggest extensions — finalists, ask AI, widen *(new 2026-05-08; deferred from v3.E menu stubs)* | Three sub-features hooking into the v3.E post-grid menu. **Finalists (`m`/`u`/`p`/`d`)** *(headline)* — mark candidates into a finalists list (`m N` / `m <name>`), unmark with `u`, view the list with `p`, run "decide" with `d` which probes auto-checks per finalist (GitHub user availability `api.github.com/users/<name>`, npm package name `registry.npmjs.org/<name>`, PyPI package name `pypi.org/pypi/<name>/json`) and renders a side-by-side comparison so the user can choose deliberately; final numeric pick from the comparison proceeds to the existing post-pick flow (defense bundle, register, next-step). USPTO / social-handle / Google-Trends pre-checks remain deferred (paid APIs / scraping concerns). **Ask AI (`?`)** — gpt-5-mini call given topic + vocab + name + question, returns 1-3 sentence explanation; default question "Why was this name chosen and how does it relate to the topic?"; cached by (topic, name, question) hash so re-asking the same question is free. **Widen (`w` / `w <guidance>`)** — LLM call passing existing names as history-dedup + optional user guidance ("shorter", "foreign roots", "more medical-specific"); returns 12-24 fresh candidates merged into the grid after going through the v3.E porn screen. All three slot back into the existing menu items 3/4/6/7. |
| **v4.B** | Interactive launcher (menu) *(was v3.E pre-2026-05-07; was v4.A 2026-05-07 → 2026-05-08)* | `portfolio` invoked with no subcommand drops into a grouped, rich-rendered menu — solves the "I can't remember the subcommand or the nested-quoting" friction. Groups: **Manage** (summary, project status, cleanup, check) · **Build** (domain suggest, bootstrap, deploy) · **Reports** (expiring, category, wip, list). Per-command flow: prompt for required positional args first, then `use defaults for everything else? [Y/n]`; if `n`, walk through optional flags one at a time. Existing per-command interactive flows (e.g. `domain suggest`'s `--browse` per-strategy round) take over after dispatch — no double-menu. After command exits, returns to menu; `q` from the menu quits. Implementation: `app(invoke_without_command=True)` with a callback that runs `menu()` from new `src/portfolio/menu.py`; nothing else changes — `make run` (no ARGS) and direct `portfolio` invocation both flow through. Bad-syntax `make run ARGS="…"` still surfaces typer's existing error (no fuzzy-match fallback in this phase). Tests use typer's `CliRunner` with mocked stdin. |
| **v4.C** | Stack detection + scaffold completeness *(was v4.B pre-2026-05-08; was v4.A pre-2026-05-07; was v3.A pre-2026-05-02)* | stack identifier (React+Vite / Astro / Python+uv / Go+Fiber / scaffold-only) · C7 vite-version-ok · C9 has-prd-md · **scaffolding-required rules: has-readme · has-gitignore · ai-agents-md-has-building-info · ai-agents-md-has-deployment-info** (every sites/* project must have the standard scaffolding produced by `/project-init` or v3 bootstrap) |
| **v4.D** | Drift + mapping *(was v4.C pre-2026-05-08; was v4.B pre-2026-05-07)* | plan-drift signal · C10 domain-dir-match (override map for harmonia / etc.) · C8 cf-pages-deployable |
| **v4.E** | Per-stack rules *(was v4.D pre-2026-05-08; was v4.C pre-2026-05-07)* | placeholder for emerging conventions: pnpm-lockfile-only · no-package-lock-json · gitignore-covers-build-output · python-uses-uv |
| **v4.F** | Remediation (second write surface) *(was v4.E pre-2026-05-08; was v4.D pre-2026-05-07)* | `portfolio project fix <name>` subcommand · dry-run by default; `--apply` required to write · `--rule R` for surgical fixes · all fixes idempotent · auto-fixes: has-prompts-md · has-ai-agents-md · has-makefile (depends on v4.C's stack identifier) · prompts-md-format · own-git-repo guided migration (parent `git rm --cached` + parent `.gitignore` entry + project `git init` + initial commit; explicit confirmation each step touching parent repo) · `--yes` skips prompts for scripted runs · templates embedded in `src/portfolio/templates.py` · `platform-declared` and `has-category` deferred (require user choice / curation) |
...
| `vite-version-ok` | Vite ≥6 for React projects | v4.D |
| `has-prd-md` | `docs/prd.md` exists | v4.D |
| `has-readme` | `README.md` exists at project root | v4.D |
| `has-gitignore` | `.gitignore` exists at project root | v4.D |
| `ai-agents-md-has-building-info` | `AI_AGENTS.md` contains a `## Building info` heading (referencing the central builder at `~/work/projects/builder/` + `../Makefile`) | v4.D |
| `ai-agents-md-has-deployment-info` | `AI_AGENTS.md` contains a `## Deployment info` heading (platform / live URL / deploy trigger) | v4.D |
| `cf-pages-deployable` | frozen-lockfile install OK; no stray gitlinks; no `_redirects` SPA fallback | v4.D |
| `domain-dir-match` | dir name matches a plan.md domain (or in override map) | v4.D |
