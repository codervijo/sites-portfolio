...
| v15.T | ✅ | **File-activity progress feed for `project translate`** — triggered by operator friction 2026-06-08: `lamill project translate mspproof.com` sat on a silent prompt for 1m24s because the port is one fully-buffered `claude` subprocess (`run_claude` → `subprocess.run(capture_output=True)` — nothing reaches the terminal until it exits). Presentation-only fix in `cli.py`; `port_to_astro` / `run_claude` untouched (the shared subprocess primitive the Tier-2 fixers also use stays byte-identical). A daemon thread polls three output buckets (`src/pages/`, `src/components/`, `public/`) for files modified at/after the run start (skipping the `.tmp` atomic-write artifacts `sweep_tmp_artifacts` later cleans, so the tally doesn't flicker) and drives a `rich` `console.status` spinner: `⠋ Porting mspproof.com… 1m24s · 7 files written · pages 3 · components 4 · public 0`. Teardown via a `threading.Event` + `join(timeout=2.0)` in a `finally`, so a failed/raised port still tears the thread down. Spinner gated on `console.is_terminal` (degrades to a no-op `nullcontext` on piped/CI output — the port runs identically). Success line folds in the final tally: `✓ Port complete. 23 files written in 6m12s · Cost: $1.20`. No real percentage bar (no known total) and no live cost (spend is only known post-exit; that needs the streaming `--output-format stream-json` path — deferred). New helpers `_fmt_elapsed` / `_count_recent_files` / `_port_progress_counts` / `_watch_port_progress` + `_PORT_PROGRESS_BUCKETS` in `cli.py`. ~75 lines, one file, no new dependency (`rich` + `threading` already in). |
...
| v26.F | ☐ | **Fleetwide audit + promote to `fail` + lock invariant.** Once v26.C/D/E ship: re-run the fleetwide CHECK_150 probe (already exists as `_classify` in the check module — can wrap as a one-off bulk runner). Expect the count to drop from `29 fail / 6 pass` toward fleet-clean. For any remaining offenders (Bucket E broken sites, no-local-repo entries): log per-site in `docs/bugs.md` with platform + reason. Once the fleet is clean (or every remaining offender has a justified exemption logged), promote CHECK_150 from `warn` to `fail` (one-line `SEVERITY` change). Update `docs/CLAUDE.md § Locked target shapes` with the apex-canonical + 308-permanent invariant + cross-link to CHECK_150 so future bootstraps inherit it. **2026-06-13: CHECK_150 severity promoted `warn`→`error`** (operator call after canonicalization bit the fleet hard; note `run()` already returned `fail` status — severity is catalog-display metadata, so this aligns the declared severity). The fleetwide bulk audit + the `docs/CLAUDE.md` lock-write remain open. |
| v26.G | ✅ | **Host-consistency triad (markup / sitemap / GSC).** 2026-06-13. CHECK_150 is network-only; these add the *consistency* layer that catches the donready coverage-break class — redirect correct, but canonical / sitemap / GSC pointing at a non-apex host: **CHECK_158 canonical-host-is-apex** (every live page's `<link rel="canonical">` host == apex), **CHECK_159 sitemap-host-is-apex** (every sitemap `<loc>` uses apex), **CHECK_160 gsc-sitemap-host-is-apex** (the GSC-registered property + sitemaps use apex — read offline from the cached `data/gsc/<domain>/` detail, no auth in the check). All `error` severity, all in lamill's catalog (reuse `_live.py` + `gsc_detail_cache`; **no rankmill split** — lamill already owns per-page canonical via CHECK_072/092). Born-compliant: the `project translate` prompt now mandates `site: "https://<apex>"` + `@astrojs/sitemap`, and the `AI_AGENTS.md` template documents the apex-only rule. Apex is the ONLY canonical host — **no `www` option** (operator-locked 2026-06-13). 20 new tests; full suite green (3318, the lone red is the pre-existing CHECK_143 drift bug). |
...
### v32 — deploy-verification honesty & resilience *(new 2026-06-05 after `lamill new deploy mdburst.com --watch` reported `✓ fully live` while the apex was still a Porkbun URL-forward (302 → l.ink) on Porkbun NS — a false-green deploy. Bundles three clustered deploy false-positives into one tier: the live-probe/watch redirect-following bug + the NS-delegation-vs-registrar-API gap (incl. Porkbun URL Forwarding) + the 2026-05-31 scopeguard.xyz `1014`/pending-verification gap (this tier absorbs that bug). See `docs/bugs.md` 2026-06-05 ×2 + 2026-05-31. **Tier complete 2026-06-05** — v32.A (ADR-0022) + v32.B/C (live-probe + NS-delegation honesty) + v32.D (Porkbun URL Forwarding detect/clear) + v32.E (apex CNAME from real subdomain) + v32.F (`pending_verification` + `--repair`) + v32.G (sitemap from robots.txt). All four clustered deploy false-positives fixed + moved to `docs/bugs.md § Fixed bugs`; ~48 new tests; full suite green (3163).)*
...
| v32.B | ✅ | **Off-domain-redirect = not-live (Bug 2).** Shipped 2026-06-05. New shared `_probe_apex_live(domain, head=)` helper classifies the followed-redirect final response via `check._classify` — `is_live` is True only for classification `live-site` (same eTLD+1). `_deploy_step8_live_probe` reports `↷ not live — forwarder/parked → <host>` instead of `✓`; `_deploy_watch_loop` sets `live_ok = apex_live` (dropped `startswith("2"/"3")`). `l.ink` added to `check.PARKED_HOST_SUFFIXES` (Porkbun URL Forwarding lands there). 8 new tests + 6 watch-loop tests updated (mocks carry `.url`). Same-site apex→www + http→https stay live; `302→l.ink` + off-domain forwarder are not-live. |
| v32.C | ✅ | **NS delegation honesty (Bug 1, Layer A).** Shipped 2026-06-05. New `_ns_delegation(domain, target)` (reads real delegation via `diagnose._dig(domain,"NS")`, normalizes trailing-dot/case, compares to target) + `_print_ns_delegation` reporter. Step 4's Porkbun **and** GoDaddy branches now print the registrar-API match *and* the real `dig NS` state as distinct lines — `✓ delegation confirmed` only when dig agrees, else `↷ NS set, awaiting delegation` (requested vs delegated shown). Propagation-aware: empty `dig` answer ⇒ `↷ not yet visible`, never a hard failure (ADR-0015). 6 tests with a stubbed `_dig`. |
| v32.D | ✅ | **Porkbun URL Forwarding detect + opt-in clear (Bug 1, Layer B — root cause).** Shipped 2026-06-05. `porkbun_dns.py`: `UrlForward` dataclass (+ `is_apex`), `get_porkbun_url_forwarding()` (`domain/getUrlForwarding/<domain>`) + `delete_porkbun_url_forward()` (`domain/deleteUrlForward/<domain>/<id>`, refuses empty id) on a shared `_post_porkbun` envelope helper. Step 4 Porkbun branch runs `_porkbun_forwarding_preflight` before the NS read: active apex forward → `↷ URL Forwarding active → pins Porkbun NS … remove at <url> or pass --clear-forwarding`. New `--clear-forwarding` flag deletes apex forwards (confirm-gated unless `--yes`, idempotent; read failures warn-not-abort). 12 tests via `httpx.MockTransport` + monkeypatch. |
| v32.E | ✅ | **Apex CNAME target from project `subdomain` (absorbs scopeguard root fix).** Shipped 2026-06-05. New `_resolve_pages_subdomain(project, slug, cf_account)` returns `(target, authoritative)`: prefers `project.subdomain`, **re-fetches** via `get_pages_project` when the just-created object doesn't carry it yet (CF assigns the suffixed `*.pages.dev` async — the create response is often empty), and only falls back to the `{slug}.pages.dev` guess as a last resort with `authoritative=False`. Step 6.5 uses it and, when guessing, prints a loud `↷ … this CNAME will 1014` warning instead of silently creating the wrong record. 5 tests (subdomain present → no refetch; empty → refetch; unreadable/error/None → flagged guess). |
| v32.F | ✅ | **`pending`/`1014` resilience + `--repair` (absorbs scopeguard resilience).** Shipped 2026-06-05. New CF helpers `get_pages_domain_status()` (GET `…/domains/{host}` → status; 404→None) + `delete_pages_custom_domain()` (DELETE; 404→False). At watch timeout, when zone+build are green but the apex still isn't serving, the loop probes the custom-domain status and returns a distinct `pending_verification` state (`✗ … stuck: pending` + a `--repair` pointer) instead of a generic `timeout` — falls back to `timeout` if the domain reads `active`. New `--repair` flag runs `_deploy_repair_custom_domain` (Step 6.6, cf-pages only): re-points the apex CNAME at the authoritative subdomain (delete-wrong + create-right) then removes + re-adds the custom domain to force re-verify, confirm-gated unless `--yes`. 9 tests. |
| v32.G | ✅ | **Sitemap URL from `robots.txt` (Bug — `@astrojs/sitemap` mismatch).** Shipped 2026-06-05. New `gsc_admin.resolve_sitemap_url(domain)` fetches the live `robots.txt` and returns its first `Sitemap:` line (case-insensitive), falling back to `https://<domain>/sitemap.xml` only when robots.txt is unreachable / 404 / has no `Sitemap:` line. Deploy Step 9 (`_deploy_step9_gsc`) now submits that resolved URL instead of the hardcoded `/sitemap.xml`. New `gsc_admin.delete_sitemap()` (idempotent, mirrors `submit_sitemap` with DELETE) clears a stale entry. Fixes the fleet-wide Astro GSC "sitemap parse errors" (`@astrojs/sitemap` emits `/sitemap-index.xml`; the SPA catch-all serves HTML for `/sitemap.xml`). 9 tests via `httpx.MockTransport`. |
...
### v33 — agent-authored site changes (`lamill project delegate <domain> "<request>"`) *(new 2026-06-05; design locked + v33.A planning 2026-06-06. Operator wants to hand a `sites/<domain>/` a slightly-complicated, multi-step instruction and have Claude implement it semi-autonomously. Reshaped 2026-06-06 from the original "one-shot host `claude -p`" into a **sandboxed, supervised, containerized** run: Claude executes **inside a fresh, disposable per-run container** (`lamill-delegate-<domain>`, built from the builder stack image; not the shared `mb1`, since the run kills its container on exit) via `docker exec` (only `sites/<domain>/` bind-mounted RW; host `~/.claude` mounted for auth — the rankmill/threadradar no-key pattern), governed by a **two-axis supervisor** (liveness = output stream flowing; progress = net diff + action novelty — catches "tokens but stuck") plus wall-clock + budget backstops; refuses on a dirty tree; verify = `make buildsh` + `project check` + Playwright-in-container visual probe; ends at an **uncommitted reviewable diff** (never auto-commits/reverts). A dedicated verb — the called-out exception to prefer-check/fix, because open-ended work has no gap to detect or green to assert. Third local-FS write surface → **ADR-0023 (Accepted 2026-06-06)**. **No `fleet delegate` for now** (operator deferred 2026-06-06); **v33.F/G/H added 2026-06-08** — a delegate ergonomics + grounding pass from the operator's `meetwhen.xyz` session: paste/pipe request input (no more shell-quoting a full prompt), a site-grounded `--append-system-prompt` with `docs/` map-on-demand, and a post-run doc trail (`Prompts.md` orchestrator-written + relevance-gated agent updates to `prd.md`/`CLAUDE.md`). **v33.F shipped 2026-06-08** (paste/pipe input + `/dev/tty` confirm); **v33.G shipped 2026-06-08** (system-prompt split via `--append-system-prompt` + `docs/` map-on-demand); **v33.I shipped 2026-06-08** (dirty-tree friction: preflight-before-paste + bootstrap gitignores `.astro/`) — reactive from the meetwhen.xyz run, with more delegate friction flagged for follow-up; **v33.J/K/L added 2026-06-08** (planned) — the delegate friction pass from continued `meetwhen.xyz` use: drop the no-value `--yes` confirm (J), a reliable paste terminator for the Ctrl-D flakiness (K), and startup visibility for the silent container-bringup wait (L, the v15.T pattern). **v33.J shipped 2026-06-08** (confirm gate removed; `--yes` now a no-op); **v33.K shipped 2026-06-08** (paste sentinel `.` + Ctrl-D fallback); **v33.L shipped 2026-06-08** (startup visibility — `on_progress` hook + CLI spinner). **Friction pass J/K/L complete.** **v33.M shipped 2026-06-08** (end-of-run output — agent summary + diff + commit command; reactive from the first real inspect-first run, where the agent's report-back was being discarded). **v33.H shipped 2026-06-08** (post-run doc trail — orchestrator-written `Prompts.md` + relevance-gated agent updates to `prd.md`/`CLAUDE.md`). **v33 tier: A–D/F/G/H/I/J/K/L/M shipped; E parked-reactive.** **v33.N shipped 2026-06-09** — fleet gitignore-hygiene backfill (`CHECK_156 astro-cache-gitignored` + fix + `CHECK_157 pnpm-lock-tracked` warn-only), the deferred existing-sites half of v33.I, triggered by the dirty-tree block recurring on meetwhen + mspproof; `fleet fix --apply` backfills the fleet. v33.E stays parked-reactive (pending real-run tuning signal). **Working order: J → K → L (friction) → H (doc trail).** **v33 tier complete 2026-06-11** — operator closed the tier after delegate proved out in real use; v33.E (supervision tuning + iterate-on-failure) dropped in favor of reactive bug-driven follow-ups.)*
...
| v33.E | dropped *(2026-06-11)* | **(Reactive) Supervision tuning + bounded iterate-on-failure — DROPPED.** Was: tune the progress-window/thresholds + richer thrash detection from real runs; optionally re-invoke Claude with verify-fail context (bounded, operator-gated). **Closed by operator 2026-06-11** — delegate has proven out in real use ("worked for me a couple of times"); per the reactive-over-proactive posture, supervision tuning + iterate-on-failure stay unbuilt until a real run produces the friction that justifies them. Operator will file bugs as found. Re-open a specific piece only when a real run motivates it. (`fleet delegate` also out of scope per operator.) |
| v33.F | ✅ | **Request input ergonomics — paste / pipe, not just inline.** Triggered by the operator's 2026-06-08 `meetwhen.xyz` session: a full multi-step prompt had to be shoehorned through the shell as one quoted positional arg (the apostrophe in `visitor's` breaks `'…'`; long prompts invite quoting errors). Make `request` optional and resolve it three ways — (1) **inline arg** → unchanged (quick one-liners); (2) **no arg + interactive TTY** → print `Paste your request, then Ctrl-D:` and read `sys.stdin` to EOF; (3) **no arg + piped stdin** (`… < prompt.txt`, heredoc) → read stdin silently (free file-input; retires the need for a `--request-file` flag). Empty-after-`strip()` → abort cleanly (`no request, aborting`), mirroring an empty git commit. **Stdin-collision fix:** paste-til-EOF consumes `sys.stdin`, so the `--yes`/confirm gate's read moves to `/dev/tty` (reopen the controlling terminal — the git pattern) so pasting the prompt *and* answering the confirm both work. Touches `project_delegate`'s arg signature (`cli.py:4387`) + the confirm read only; `run_delegate` / `build_delegate_prompt` / container path unchanged — `request` still flows verbatim into `build_delegate_prompt` (`delegate.py:458`). Tests: inline still works · TTY-paste reads to EOF · piped-stdin reads silently · empty aborts · confirm reads `/dev/tty` not stdin. **Shipped 2026-06-08** — `request` Argument now optional (`[REQUEST]`); helpers `_resolve_delegate_request` / `_delegate_confirm` in `cli.py`; 13 tests (`test_delegate_v33f.py`); suite 3240 → 3253. |
| v33.G | ✅ | **Site-grounded system prompt — `--append-system-prompt` + docs map-on-demand.** Today `build_delegate_prompt` (`delegate.py:340`) folds 4 guardrail lines + a `=== SITE CONTEXT ===` block (AI_AGENTS.md + package.json + pyproject.toml, capped ~5KB via `fix_helpers.project_context`) into the single `claude -p` body. v33.G (a) **promotes** that context + guardrails to `--append-system-prompt` (the `DockerBackend.stream` cmd at `delegate.py:609`), keeping `=== REQUEST ===` as the user turn — cleaner separation, conventions weighted in the system slot; and (b) **widens grounding to the site's `docs/` as a map, not a slurp**: include AI_AGENTS.md content + a *listing* of `docs/` filenames (cheap), plus a system-prompt line instructing the agent to read the relevant docs itself via its `Read`/`Glob`/`Grep` tools (it already has them in-container). Avoids paying full-tree token/budget cost every run; scales to any `docs/` size; preserves budget for the actual work. Tests: system prompt carries guardrails + AI_AGENTS.md + `docs/` listing · request stays in the `-p` user turn · empty/absent `docs/` degrades cleanly · `--append-system-prompt` wired into the docker cmd. **Shipped 2026-06-08** — `build_delegate_prompt` split into `build_delegate_system_prompt` + `docs_listing`; backend `stream` seam gains `system_prompt`; `DockerBackend._claude_cmd` extracted + wires `--append-system-prompt`; 10 tests (`test_delegate_v33g.py`); suite 3253 → 3263. |
| v33.H | ✅ | **Post-run doc trail — `Prompts.md` (orchestrator) + `prd.md`/`CLAUDE.md` (agent, relevance-gated).** A clean delegate run should leave a trail in the site's own docs, not just a `git diff`. Two writers: (1) **`docs/Prompts.md` — orchestrator-written, deterministic.** After the verify gate passes (success only), lamill appends a dated entry in the file's documented append-only format (`## YYYY-MM-DD — delegate: <title>` / `> <request or one-line summary> · N files · $cost`). lamill owns this because it knows the exact request / date / cost / files and the parseable `## YYYY-MM-DD` H2 that `project check` reads for "last AI prompt per project" — not trusted to the agent to remember or format. Creates `Prompts.md` from the standard skeleton if absent. (2) **`docs/prd.md` / `docs/CLAUDE.md` — agent-written, relevance-gated.** A system-prompt instruction (rides on v33.G) tells the agent: if the change meaningfully alters *what the site does* → update `docs/prd.md` (behavior/features); if it alters *structure/conventions* → update `docs/CLAUDE.md`; cosmetic/copy-only → skip. Sites carry **no `architecture.md`** (confirmed across the fleet 2026-06-08 — standard site doc set is `CLAUDE.md` / `prd.md` / `growth.md` / `Prompts.md`), so the spec + conventions docs are the targets. Both updates land in the **same uncommitted reviewable diff**, reviewed + committed together — consistent with v33's never-auto-commit contract. Tests: Prompts.md entry appended in-format post-verify · skipped on verify-fail/stall · absent `Prompts.md` created · agent-routing instruction present in the system prompt · doc update surfaces in the diff. **Shipped 2026-06-08** — `append_delegate_prompt_log` (creates `Prompts.md` from skeleton if absent; deterministic `## YYYY-MM-DD — delegate` entry with summary · files · cost), wired in the CLI on a `done` run with changes (and folded into `changed_files`); `build_delegate_system_prompt` gained the relevance-gated prd/CLAUDE routing instruction (+ "don't touch Prompts.md"). 6 tests (`test_delegate_v33h.py`); suite 3277 → 3283. |
| v33.I | ✅ | **Delegate dirty-tree friction removal.** Reactive, from the operator's 2026-06-08 `meetwhen.xyz` run: the v33.B dirty-tree refusal fired *inside* `run_delegate` — i.e. **after** v33.F collected the request — so a refusal meant re-pasting the whole multi-step prompt; and the blockers were untracked generated files (`.astro/` cache, lockfile, IndexNow key) that plain `git stash` doesn't clear. Two fixes: (1) **preflight before paste** — `project delegate` now runs `preflight(domain, force=force)` (resolve site dir + clean-tree check) BEFORE `_resolve_delegate_request`, so a dirty tree (or missing site) refuses instantly with the full message and the operator never wastes a paste; the resolved `site_dir` is reused for the verify baseline (drops a redundant `resolve_site_dir`). (2) **bootstrap gitignores `.astro/`** — `bootstrap._gitignore()` now ignores Astro's generated cache, so fresh sites don't accumulate untracked junk that trips the dirty-tree precondition fleet-wide (backfill of existing sites is separate). 3 tests (`test_delegate_v33i.py`: gitignore-has-astro · dirty-refuses-before-request · preflight-strictly-before-resolve); suite 3263 → 3266. **Operator flagged more delegate friction remains beyond these two (2026-06-08) — captured for follow-up.** |
| v33.J | ✅ | **Drop the pre-run confirmation gate.** Operator-decided 2026-06-08: the `--yes` confirm adds no value for delegate. The write-surface confirm pattern (architecture.md § write-surface gates) is for surfaces that *apply* irreversible changes; delegate ends at an **uncommitted diff the operator reviews before committing**, so the prompt guards nothing — and by the time it fires, the operator has already typed the command *and* supplied a request (intent is not in doubt). Remove the confirm block from `project_delegate`; remove the now-dead `_delegate_confirm` helper + its v33.F tests; **keep `--yes`/`-y` as an accepted no-op** (operator muscle memory — removing it would error on `delegate … --yes`, the opposite of the goal). Doc: architecture.md § write-surface gates gains a delegate exemption note (safety = review-the-diff + ADR-0023's sandbox/supervisor/verify, not a pre-confirm) — **decision: a note, not a new ADR** (ADR-0023's safety model is unchanged; we're removing a redundant prompt, not a safety layer). Tests: `delegate <domain>` runs without prompting · `--yes` still parses (no-op). **Shipped 2026-06-08** — confirm gate + dead `_delegate_confirm` helper removed; `--yes` help reworded to deprecated-no-op; architecture.md § write-surface gates notes the exemption. 3 tests (`test_delegate_v33j.py`); 9 obsolete `_delegate_confirm` tests dropped from `test_delegate_v33f.py`; suite 3266 → 3260. |
| v33.K | ✅ | **Reliable paste terminator (Ctrl-D flakiness).** v33.F reads stdin to EOF, but a terminal only closes on Ctrl-D *at line start* — so a paste **without** a trailing newline needs a second Ctrl-D, which reads as "sometimes works, sometimes not." Fix: accept a **sentinel terminator** — read lines until a lone `.` on its own line, with EOF/Ctrl-D kept as a fallback. Banner becomes `Paste your request, then a line with just '.' (or Ctrl-D):`. Piped stdin (`< prompt.txt`, heredoc) is unaffected — it still reads to EOF, no sentinel needed. The sentinel line is stripped, never part of the request. Tests: lone-`.` ends input · EOF fallback still works · sentinel not folded into request · piped read unaffected · a request line that merely *contains* a dot isn't mistaken for the terminator. **Shipped 2026-06-08** — `_resolve_delegate_request` reads lines until a lone `.` (EOF fallback) on a TTY; piped stdin unchanged; banner reworded. 6 tests (`test_delegate_v33k.py`); suite 3260 → 3266. |
| v33.L | ✅ | **Startup visibility (the silent wait).** After `▸ delegate … running in sandbox…` there's a long dead gap: `backend.start()` does a `docker run` (+ a first-time `node:20-bookworm` image **pull** if uncached — a minute+), then `npm i -g @anthropic-ai/claude-code`, then `claude` boots, reads context, and plans — all before the first stream event, all silent. Apply the **v15.T pattern**: emit phase markers during `start()` (`↷ pulling image… ↷ starting container… ↷ installing claude…`) and a heartbeat/spinner until the first stream-json line (`⠋ agent starting… Ns`). Presentation-only; the supervisor/stream contract is unchanged. Tests: `start()` surfaces phase markers · heartbeat ticks until the first event · degrades on non-TTY (no spinner spam in logs). **Shipped 2026-06-08** — `run_delegate` gained an optional `on_progress(kind, detail)` hook (phase/action: starting-sandbox → agent-starting → per-tool_use → verifying); CLI drives a `rich` spinner (animates through the blocking `start()`); delegate.py stays console-free. 3 tests (`test_delegate_v33l.py`); suite 3266 → 3269. |
| v33.M | ✅ | **End-of-run output — agent summary + diff + commit command.** From the operator's first real inspect-first run (2026-06-08): a `Task 0 — inspect, report back, wait for OK` prompt correctly produced **no file changes**, but delegate discarded the agent's actual report — the *deliverable* of a report-back run — leaving only `(no file changes)`. Two fixes: (1) **surface the agent's closing summary** — `parse_stream_line` captures the `result` event's `result` text into `StreamEvent.text` → `run_delegate` → `DelegateResult.summary`; `_render_delegate_result` prints it (capped ~6KB), so inspect-first/report-back runs are actually useful. (2) **diff + commit command for runs with changes** — replaces the single `review:` line with a `diff:` command + a ready-to-paste `commit:` (`git -C sites/<d> add -A && … commit -m "delegate: <first-line snippet>"`), emitted `soft_wrap` so the long command copy-pastes intact (no rich hard-wrap mid-command); `_suggested_commit_msg` neutralizes quotes; lamill still never commits. 8 tests (`test_delegate_v33m.py`); suite 3269 → 3277. |
| v33.N | ✅ | **Fleet gitignore-hygiene backfill — the deferred half of v33.I.** v33.I fixed the bootstrap `.gitignore` *template*, but existing sites were left: they accumulate untracked `.astro/` that blocks `project delegate`'s dirty-tree gate (hit on meetwhen then mspproof 2026-06-08/09), and some carry an untracked `pnpm-lock.yaml` that passes `CHECK_031` (file-presence only) yet isn't version-controlled — a reproducibility gap behind a green check. Delivered as checks (+ one fixer) so `fleet fix` is the backfill autopilot per [[feedback_prefer_check_fix_delivery]]: (1) **`CHECK_156 astro-cache-gitignored`** (stack/warn) — for Astro sites (`[stack].framework == "astro"`, falling back to detecting `astro.config.*` when `[stack]` is absent, per the additive-optional invariant; skips non-Astro / non-web), fails when `.gitignore` lacks `.astro/`; **`fix_tier_1`** appends the `# Astro` block (idempotent). (2) **`CHECK_157 pnpm-lock-tracked`** (stack/warn) — warns when `pnpm-lock.yaml` is present but git-untracked, with an operator-action hint (`git -C sites/<d> add pnpm-lock.yaml && commit`); **no auto-fix** (staging/committing is a shared-state git op outside the file-editing fixer model). `fleet fix --apply` sweeps the `.astro/` half fleet-wide; the lockfile half stays operator-action. Tests: astro-gitignored pass/fail/skip-non-astro + `[stack]`-absent fallback + fix appends + idempotent · lockfile-tracked warn-untracked / pass-tracked / skip-absent. **Shipped 2026-06-09** — `CHECK_156` (+ `fix_tier_1`, registered fixable) + `CHECK_157` (warn-only) in `checks/stack/`, gated like `CHECK_036` (declared-stack → `astro.config.*` heuristic fallback). 11 tests (`tests/checks/test_v33n_gitignore_hygiene.py`); suite 3283 → 3294. Run `lamill fleet fix --apply` to backfill the `.astro/` half fleet-wide. |
...
**v33.F — why stdin + `/dev/tty`, not a `--request-file` flag.** The trigger was
real friction: a full multi-step prompt won't pass cleanly as one shell-quoted
positional (apostrophes, length). The temptation is a `--request-file PATH` flag,
but piped-stdin already subsumes it (`delegate <domain> < prompt.txt`) *and* gives
the interactive paste path for free — one resolution rule (`inline ▸ stdin`) covers
all three input modes, so no extra flag earns its keep. The non-obvious bit is the
**stdin collision**: reading the request to EOF exhausts `sys.stdin`, so the
confirm prompt can no longer `input()` from it. Reading the confirm from `/dev/tty`
(the controlling terminal, the way git reads y/n after a piped commit) is the clean
fix — keeps the plan-then-confirm gate intact even when the request arrived via
paste/pipe. `--yes` still skips the gate entirely.

**v33.G — map-on-demand beats slurp because the agent has filesystem tools.** The
naive grounding move is to read all of `docs/` into the prompt. Rejected: the
in-container agent already holds `Read`/`Glob`/`Grep` over the whole site dir, so
it can fetch any doc it needs — pre-loading the tree pays full token cost every run
for files most changes never touch, and delegate is *budget-capped*, so context
bloat directly steals from the work budget. So v33.G gives the agent a cheap **map**
(AI_AGENTS.md content + a `docs/` filename listing) and an instruction to read what's
relevant, rather than the territory. Promoting the block to `--append-system-prompt`
is the other half — conventions belong in the system slot, the request in the user
turn.

**v33.I — preflight is a gate, so it belongs before the gathering.** The
ordering bug was subtle: v33.F made the request optional (paste/pipe), and the
dirty-tree check lived inside `run_delegate`, so the natural flow collected the
(possibly long, pasted) request *first* and only then discovered the tree was
dirty. The fix is a sequencing principle — cheap, certain refusals (docker
present, site exists, tree clean) run *before* any expensive or interactive
step (the paste, the confirm, the container). The second fix attacks the same
friction at the source: `.astro/` was never gitignored, so it accumulated as
untracked cache that tripped the clean-tree gate — a bootstrap omission, fixed
in the template (existing sites backfill separately). Both came straight from
watching a real run; the operator noted further delegate friction beyond these,
held for a follow-up pass.

**v33.J/K/L — the delegate friction pass (reactive, from real meetwhen.xyz use).**
Three pains surfaced once delegate was driven for real, and they share a theme:
the ceremony around the actual work was heavier than the work. **J (drop the
confirm)** — the write-surface confirm is a safety primitive for surfaces that
*apply* changes; delegate's output is a reviewable uncommitted diff, so the prompt
guards nothing the review doesn't already. Removing it is subtractive, not a new
feature. **K (paste terminator)** — the Ctrl-D unreliability isn't flakiness, it's
terminal line-discipline: EOF only fires at line start, so a no-trailing-newline
paste needs a second Ctrl-D. A sentinel line (`.`) makes termination deterministic
regardless; Ctrl-D stays as a fallback. **L (startup visibility)** — the silent gap
is container bringup (image pull + claude install + boot/plan); the fix is the v15.T
pattern (phase markers + heartbeat), reused not reinvented. **Deferred, not dropped:**
opt-in auto-stash for a dirty tree (#4) and prompt-replay across retries (#5) — the
operator didn't pick auto-stash, and piped stdin (`< prompt.txt`) already covers
retry-without-re-paste, so both wait for a clearer signal. These ship before v33.H
because friction-in-the-loop outranks the doc-trail nicety.

**v33.M — the deliverable isn't always a diff.** The first real inspect-first
run exposed a wrong assumption baked into delegate's output: that the *result*
of a run is its *file changes*. A `Task 0 — inspect, report back, wait for OK`
prompt produces zero file changes by design — its deliverable is the agent's
*analysis*, which delegate streamed and then discarded, leaving `(no file
changes)` and nothing to act on. Capturing the `result` event's text into
`DelegateResult.summary` and always printing it makes report-back runs
first-class. The diff/commit-command half is the smaller, asked-for part — with
one non-obvious wrinkle: a long `git … add -A && … commit -m "…"` line must emit
`soft_wrap` so rich doesn't hard-wrap a newline into the middle of a command the
operator will copy-paste. lamill still never commits — it hands over the command.

**v33.N — backfill the convention, don't just template it forward.** v33.I
fixed the bootstrap `.gitignore` template, which silently assumed new sites were
the only ones that mattered — but the fleet is mostly *pre*-v33.I sites, and the
delegate dirty-tree gate is what exposed it (twice). The durable fix is a check
the existing catalog enforces, not a one-off cleanup, so `fleet fix` keeps the
fleet converged. The `.astro/` half is a clean file-edit fixer; the
untracked-lockfile half is deliberately *not* auto-fixed — staging or committing
is a shared-state git operation, and the project's fixers only ever edit working-
tree files (a fixer that runs `git add` would leave a staged-uncommitted index
and blur the "review the diff, then commit" contract). So it warns with the exact
recovery command and lets the operator commit. Note `CHECK_031` already passed on
...
H2) — exactly the kind of thing to write mechanically from lamill, not to hope the
agent remembers and formats correctly. The `prd.md`/`CLAUDE.md` update is a
**judgment call** (is this change architecturally/behaviorally significant, and
what should the note say) with full diff context — exactly what the agent is placed
to do, gated on relevance so trivial changes don't spam docs. Both land in the one
reviewable diff, so the operator still sees and can adjust every doc write before
committing — the never-auto-commit contract is preserved. Note sites have **no
`architecture.md`** (confirmed across the fleet 2026-06-08), so the routing targets
the docs sites actually carry; introducing an `architecture.md` per site was
considered and rejected as imposing a new convention on 30+ sites for marginal gain.
...
| v31.E | ✅ | **Hotfix — GoDaddy NS update used PUT (404s); must be PATCH.** Surfaced 2026-06-09 on `iotbastion.com`'s `new deploy` Step 4: `set nameservers … HTTP 404 NOT_FOUND`. `GET /v1/domains/{domain}` succeeds (domain readable, ACTIVE) but `set_nameservers` issued `PUT /v1/domains/{domain}` — GoDaddy registers no PUT on the domain resource (updates go through **PATCH**, `nameServers` in the DomainUpdate body), so it 404'd on *every* GoDaddy domain; the v31.C write-path never actually worked. Shipped green because the v31.C test asserted `method == "PUT"` against a mock transport that accepts any verb. Fix: `c.put` → `c.patch` in `godaddy.py:set_nameservers`; test renamed + asserts PATCH (regression guard). **Needs live validation** on a real GoDaddy domain (mock confirms lamill sends PATCH, not that GoDaddy accepts it). See `docs/bugs.md` 2026-06-09. |
...
posture from v12. **v35 CLOSED (2026-06-08):** A–D + F (cli.py split, 11,095→5,848)
+ G (dead-code) shipped; E (deploy extraction) and the remaining MEDIUM/LOW
register items dropped — see per-phase rows.
...
| v35.B | ✅ | **Shared HTTP layer + transient/permanent taxonomy (H4 + H7).** New `_httpapi.py` (`managed_client()` ctx + `raise_for` + `HttpApiError`/`TransientHTTPError`/`PermanentHTTPError` + canonical `RETRYABLE_STATUSES`) so the `↷`/`✗` color-code rule is enforceable fleet-wide. **ADR-0024.** Providers keep typed error classes, reparented onto the taxonomy. Migrated all five named clients — `godaddy`, `vercel`, `cloudflare` (25 sites; v32 ReadTimeout `try/except` nested in the `with`), `ga4_admin`, `gsc_admin` (managed_client also fixed an error-path client leak in the GA4/GSC `finally: pass` pattern; `verify_domain`'s poll loop keeps its explicit all-paths close by design); `serp` sources the retryable set. 26 foundation tests; full suite green (3234) after each increment. **Opportunistic remainder** (`gtrends`/`indexnow`/`porkbun_dns`) folded into v35.G. |
| v35.C | ✅ | **Stack-detector dep-primitive consolidation (H3).** Extracted shared `read_package_json` + `merged_deps` into `stack_classifier` (the canonical module); `stack_translate.detect_stack`/`validate_translation` + `bootstrap.detect_stack_from_pkg` now read deps through `merged_deps` (was copy-pasted 4×). **Finding revised:** the three detectors are *disjoint consumers* with intentionally different vocab + looseness (fleet drift-check `tanstack`/`wordpress` vs Lovable-port `tanstack-start`/`package_json_unreadable` signal vs new-bootstrap loose `react→vite`) — they don't run interchangeably, so H3 is **duplication debt, not an active correctness bug**. Full vocab unification rejected (would break each consumer's contract + tests for no operator-facing gain). 4 primitive tests; suite green (3238). |
| v35.D | ✅ | **Silent-swallow fixes (H6) + bs4 wrap.** `diagnose.probe_inventory` now records a throwing data-load on a new `InventoryLayer.errors` field (rendered as `↷`), so a broken snapshot is distinguishable from "domain absent" (was 3× bare `except: pass`). `research_gates.suggest_reductions` surfaces the cause (missing key / LLM fail) on stderr instead of a silent `[]`. Wrapped the 3 `bs4` lazy imports behind one `_bs4()` helper (typed `uv sync` hint). **Verified on read:** `decide.py` already captures the cause (`backend="skipped", error=...` — not silent) and `content_derive._call_openai` *propagates* (`raise_for_status` — not a swallow); both were H9/H10 untyped-HTTP findings, out of v35.D scope. 6 tests; suite green (3238). |
| v35.E | dropped *(2026-06-08)* | **`deploy_pipeline.py` extraction (H2) — DROPPED.** Was: extract `_deploy_cf_unified`'s inlined steps into `_deploy_stepN_*(*, ctx)` + a `DeployContext` dataclass. **Dropped by operator decision** closing out v35: the deploy pipeline is the only *outward-facing* path (real GitHub repos / CF projects / DNS / registrar NS) and the test suite is mocks, so a behavior-preserving refactor can't be safely validated without repeated real deploys — cost/risk not worth the cleanup. The pipeline **works as-is** and stays monolithic in cli.py; the `locals().get("project")` state hack + threaded-locals debt is acknowledged and accepted. Re-open only if the deploy path itself needs substantial functional change. |
...
| # | Status | Feature |
|---|---|---|
| v37.A | ✅ done | **Kickoff / decisions + ADR.** **Survey DONE → `docs/coding-agents-survey.md`; picked OpenHands** (operator 2026-06-13, respecting "no codex"): among OSI-permissive, headless, container-friendly options it covers the most of our six "misses" natively (real `str_replace` edit tool + JSONL events/exit-codes that mini-swe-agent lacks). Remaining to lock: the **agent-adapter interface** (install cmd · headless argv builder · output→`StreamEvent` parser · error/exit mapping — see v37.B); the fallback policy + flag (`--backend {auto,claude,oss}`, default `auto`); the guardrail division (wrapper owns sandbox/firewall/supervisor/verify/diff-review; agent owns the model-loop); **smoke-test OpenHands' exact invocation in a throwaway container** (confirm dirty-tree-not-committed + the JSON schema) before building. **ADR** extending ADR-0023. |
| v37.B | ✅ done | **`OSSAgentBackend(DelegateBackend)` + `OpenHandsAdapter`.** Build the backend **parameterized by a small agent adapter** (`install cmd · headless argv · output→StreamEvent parser · error/exit mapping`) so the wrapped agent is swappable — OpenHands first; Codex / mini-swe-agent are future adapters (see the survey's revisit triggers). Install + run the agent headless per sub-task (`openhands --headless --json -t "<subtask>"` in `/work`, `--override-with-envs`, model + `OPENAI_API_KEY` via the apikeys pathway, **don't mention git in the task** — its bash will commit); map OpenHands' JSONL events → `StreamEvent`s. Implements `start`/`stream`/`kill`/`exec`/`last_run_evidence`. **Honest error bubbling (reuses v33.O):** from OpenHands' exit codes 0/1/2 + stderr + JSON error events, surface the *real* cause via `last_run_evidence` + the `diagnose_no_result`-style ranked reason, **never a generic "no result."** Mock-tested with a fake agent process (no real API). |
| v37.C | ✅ done | **Fallback wiring + selection.** `--backend {auto,claude,oss}` (default `auto`); `oss` runs the configured adapter (OpenHands by default); `auto` = Claude-primary with an **OSS-agent hand-off on a hard cap** — fires **mid-run *and* at pre-flight** (already-capped-at-start hands off immediately, no wait — the original miss the operator caught 2026-06-14). Continues the in-tree partial (resume-on-cap's tree-as-memory). For the OSS/OpenAI path a 429 is a short backoff, not a 5h cap, so the quota-wait machinery mostly no-ops; split + verify gate unchanged. |
| v37.D | ✅ done | **Live validation.** Real OpenHands smoke-run end-to-end in the throwaway container (exit 0, edited the target file, left a dirty/uncommitted tree — confirms the v37.D contract + the JSONL schema). Auto cap-hand-off (Claude→OpenHands) validated by `tests/test_delegate_v37.py` (mid-run + pre-flight). Full real Claude→OpenHands hand-off on a live airsucks sub-task pending operator re-run with the fix. |
| v37.E | ✅ done | **Baseline-build gate (env-hardening).** Build the **pristine** tree in the sandbox *before* the agent runs (same cmd as the verify gate, `_detect_build`); if it fails, the *environment* is broken (wrong PM, missing dep, bad config) — bail with the real build error instead of letting the agent loop on a phantom (the airsucks 5-hour burn). Gated on a clean tree (`changed_files` empty) so it runs only on the first attempt — resumes/later sub-tasks aren't pristine. `--no-baseline` opt-out. New `_baseline_build`; `baseline_check` threaded through `run_delegate`/`_resilient`/`_split`. 5 tests. *(Pairs with the v37 batch's `_detect_build`→pnpm fix, the actual root cause.)* |
| v37.F | ✅ done | **Prompt upgrade.** `build_delegate_system_prompt` now states the **build contract** (the exact PM + `<pm> run build`; never switch PMs), an **anti-thrash inoculation** (*"the project was confirmed to build cleanly before you started; an unrelated error is environmental — say so, don't loop on config"*), and **injects `CLAUDE.md`** (previously a write-target only, so the agent never saw the conventions) **+ prior `docs/delegate-notes.md` learnings**. Steers the agent on top of the now-sound (v37.E) environment. Tests in `test_delegate_v33g.py`. |
| v37.G | ✅ done | **Circuit-breaker — bail when the agent is stuck on its build.** Trips on **N consecutive build failures** (the v37.H signal, no passing build between) → kills the run with status `build-stuck`. Refined from the original "one-strike + flat-diff" policy: flat-diff would've *missed* airsucks (the agent was *churning* the config, not stalled), and literal-1 false-trips on every run's normal first build hiccup — so the trip is the *stuck* point: `Bounds.build_fail_limit` (default **3**; `0` disables). The baseline gate (v37.E) already proved the env builds, so repeated failures here = the change/approach, not the environment; the result `message` (v37.H) names it. 4 tests. *(Hand-off-to-OSS on `build-stuck` left as an optional follow-up — the fast honest bail is the value.)* |
| v37.H | ✅ done | **Parse the agent's own build outcomes from its stream (observability).** Closes the hole that let airsucks burn 5h without naming the build: the agent builds inside its run to check its work, but lamill saw only opaque tool-use, never "the build failed" (the verify gate names build failures, but only after a *clean finish* — a mid-thrash cap never reaches it). `classify_build_line` scans each raw stream line for distinctive build success/failure signatures (works for claude + OpenHands). `run_delegate` surfaces failures **live** (`⚠ the agent's own build is failing ×N`) and **names them in the result** (`message`: "build failed ×N — stuck on the build; baseline passed, so it's the change not the env"). Fixes the supervisor's blind spot too (config-edit churn looked like progress — now there's a real build-failing signal, which v37.G can trip on). 6 tests. |
...
### v41 — deploy / GSC observability (catch silent build failures + sitemap (re)submit) *(new 2026-06-14 — reactive; surfaced when an airsucks CF build failed silently for hours and no lamill check noticed)*

#### Phases

| # | Status | Feature |
|---|---|---|
| v41.A | ✅ done | **`settings gsc submit-sitemap --site <domain>` command.** Thin wrapper over `gsc_admin.submit_sitemap()` (which existed but was only reachable inside `new deploy` Step 9). Resolves the sitemap URL from the live `robots.txt` `Sitemap:` line (v32.G); idempotent by default; `--force` deletes-then-resubmits to force a GSC re-fetch after a content change (e.g. enabling prerendering); `--url` overrides. `✓/↷/✗` markers; `GSCAdminError` → exit 2. 5 tests (`tests/test_settings_gsc_submit_sitemap.py`). |
| v41.B | ✅ done | **Deploy-health check via the CF deployments API.** Rewrote `CHECK_145 deploy-fresh` to read the **CF Pages deployments API** (build status + the deployed commit) instead of `/version.json` — which was never rolled out (0/39 served it), so 144/145/146 were dead fleet-wide. The check now: `fail` if the latest CF build **failed** (the silent-build-failure case), `fail` if the deployed commit ≠ `origin/<prod-branch>` (latest pushed commit didn't ship), `pass` if current, `warn` for non-cf-pages / no-creds / unresolvable. cf-pages-only (the fleet is uniform cf-pages after the 2026-06-14/15 Worker→Pages consolidation); resolves the project name via both fleet conventions (slug + dashed-domain). New `cloudflare.latest_pages_deployment`. 11 tests; verified live (all migrated sites pass). **Follow-up:** `CHECK_144 has-version-stamp` + `CHECK_146 last-build-success` are now obsolete (version.json abandoned) — retire in the next bug-scrub alongside the 3-parser consolidation. |
...
| v45.F | ☑ | **Registrar-truth gap handling.** *(shipped 2026-09-08; renumbered ×2 — was v45.D, then v45.E; 21 tests.)* Client domains carry `registrar="other"` and a genuinely-unknown `auto_renew` (the operator holds no credential for a client's registrar account). `fleet domains --expiring` now renders `unknown (client)` rather than a blank cell, which read as "off" or as a data bug. **Expiry is fetched, not suppressed** — the original design said mute it for lack of registrar truth, but `availability.rdap_expiry_date()` (a sibling of `rdap_creation_date` over a new shared `_rdap_event_date`) reads the RDAP `expiration` event with no credential at all. `fleet sync --refresh-rdap` fills `expires` for `owner != "lamill"` domains only — registrar truth stays authoritative for the operator's own — and **re-fetches every run** (unlike `domain_created`, expiry moves on renewal, and a stale value defeats the signal). `fleet focus` keeps the ⚠️ expiry signal at `_RANK_RED` for client domains and retargets only the action line to `→ client-owned (<name>) — ask them to renew`. **Muting was the wrong default:** a client quietly letting the domain lapse kills the site and reads as the operator's fault — it is the one failure in model (A) that most needs a warning, not the one that least does. |
...
**Ordering: fix-first, enforce-last.** Capability → fixer → deploy → gated backfill (fleet goes green) → *then* flip the check. The check goes strict only once the fleet is already conformant, so no site shows a 🔴 that isn't already fixed. We don't need the red to *drive* the work — the exact work-list is already known (snapshot `data/checks/2026-07-07.json`).

| # | Status | Feature |
|---|---|---|
| v46.A | ☑ | **Kickoff / decisions lock.** *(shipped 2026-07-17 — ADR-0027 Accepted; decisions locked below.)* Lock: apex stays canonical (**not** reversed) but `www` becomes *required* + 301/308→apex, tightening the doc-only `🔒 apex-canonical` shape; the CF mechanism = **per-zone proxied `www` DNS record + a zone-level Single Redirect Rule** (Rulesets API, `http_request_dynamic_redirect`), over account-level Bulk Redirect, for per-zone idempotent `fleet fix`/`new deploy` units; redirect status **301** (CHECK_150 accepts 301\|308); **fix-first ordering** (flip the check LAST, v46.F, after backfill); **scope + exclusions** (mandate fires only where apex is a live 200; parked/archived/dark sites excluded via the existing `fleet fix` seams — no new v46 list); rollout gating (dry-run→operator go, never auto-run); **ADR-0027**. *(Deliverable = ADR-0027 + this tier; completes on operator sign-off.)* |
| v46.B | ☑ | **CF redirect-rule capability in `cloudflare.py`.** *(shipped 2026-07-17 — `www_redirect_rule`/`get`+`put_dynamic_redirect_rules`/`ensure_www_dns_record`/`ensure_www_redirect`/`ensure_www_redirects_to_apex` + `WwwProvision`; 19 tests. Live-confirmed the mechanism off `voltloop.site`: proxied `CNAME www→apex` + CF-edge 301, no `_redirects`. **Token gap found:** current `CF_API_TOKEN` 403s on the Rulesets endpoint — needs Dynamic-Redirect edit scope before v46.C/E run live.)* New `ensure_www_redirect` over a get-or-create Rulesets helper for the zone's `http_request_dynamic_redirect` ruleset (upsert `www.<apex>`→`https://<apex>` 301, preserve query); idempotent GET-then-PUT per the existing CF-fixer pattern; proxied `www` DNS via the existing `create_dns_record`. **Mirror the rule shape off a live already-conformant zone** (e.g. `voltloop.site` — one of the 11 that already redirect www→apex) rather than guessing the API shape. Unit-tested against `MockTransport`. No behavior change until a caller uses it. |
| v46.C | ☑ | **Extend the CF fixer** *(shipped 2026-07-17 — `_apply_cf_canonical` combiner: apex-live scope guard + `_apply_cf_always_use_https` (unchanged) + new `_apply_cf_www` + `_merge_fix_results`; `cloudflare.www_dns_present`/`www_redirect_present`; 403 scope-gap → actionable Dynamic-Redirect hint; 12 new tests, full suite green except 1 pre-existing unrelated CHECK_143 failure).* ~~(`_apply_cf_always_use_https` → broadened).~~ On top of the HTTPS toggle, ensure the proxied `www` record + www→apex redirect exist. **Scoped to apex-live sites** (skip when apex ≠ 200 — nothing to redirect to) and honors the existing `dark_sites`/archived filters `fleet fix` already applies. Probe-before-act; "already exists" → `nothing-to-do`; the 4 www-serves-200 second-canonical sites get converted to a redirect; verify by re-probing `www`→301. Dry-run default; `--apply` commits. Vercel branch already does www→apex 308 — unchanged. |
| v46.D | ☑ | **`new deploy` born-conformant.** *(shipped 2026-07-17 — extracted `_deploy_step_www_redirect` helper = Step 6.7 after the apex-DNS step; calls `ensure_www_redirects_to_apex` (idempotent, zone-level for Pages+Workers); soft-fails ↷ + Dynamic-Redirect scope hint on any CF error so the deploy never blocks (ADR-0015); 5 tests; 45 end-to-end deploy tests still green.)* ~~Add the www-record + redirect step to `_deploy_cf_unified`.~~ Add the www-record + redirect step to `_deploy_cf_unified`, idempotent per ADR-0015 (probe before act; catch "already exists"). New sites never enter the fleet with broken/absent `www`. |
| v46.E | ☐ | **Fleet backfill rollout — targeted runner, bypasses the fail-gate** (decided 2026-07-17). `fleet fix` only runs fixers on *failing* checks (`fix_cli.py:79`), but fix-first keeps CHECK_150 passing the no-www sites until v46.F — so the backfill is a dedicated one-shot that runs the CHECK_150 www fixer directly against the known apex-live-but-no-www sites, independent of check status (preserves the no-red-window). **Gated: dry-run → operator reviews diff → explicit go; never auto-run.** Targets the ~33 NXDOMAIN sites + the 4 second-canonical conversions; skips the parked outliers (`lamill.us`, `lamillrentals.com`, `virtually.co.in`) via the exclusion seam. **Prereq: the CF token needs the Dynamic-Redirect edit scope added first** (v46.B finding). |
| v46.F | ☐ | **Flip `CHECK_150` (LAST).** Rule 2 `www.status is None → pass` becomes `→ fail`; rewrite the docstring lines + pass-message that bless absent-www; add tests for the new fail case + the still-passing www-308/301 case. Lands **after v46.E** so the fleet is already conformant — the flip surfaces only genuine residual gaps, not a wall of expected red. On completion: BUG-087 Open→Fixed; update `docs/CLAUDE.md § 🔒 apex-canonical` shape text + memory `project_apex_canonical_locked`. |

#### Design notes

**Decision (2026-07-16, operator): apply apex-canonical universally *and* make `www` redirect instead of NXDOMAIN — without reversing apex-canonical.** Apex stays the sole canonical host (still the only 200, still the only `<loc>`/`<link rel=canonical>` host). What changes is that `www` must *exist and 301→apex* rather than being allowed to not exist. The universal target for all in-scope CF sites: `https://<apex>/`→200, `https://www.<apex>/`→301→apex, `http://<apex>/`→301→https. See **ADR-0027**.

**Real fleet state (snapshot `data/checks/2026-07-07.json`, 51 CF domains).** Apex: 49/51→200 (outliers `lamill.us` 404, `virtually.co.in` SSL-broken). www splits four ways: **33 NXDOMAIN** (no www record — the true "no www" set; flip red under the new rule → create www+redirect), **11 already www→apex redirect** (already conformant; stay green — and the reference for v46.B's rule shape), **4 www-serves-200** as a second canonical (`linkedcsi.live`, `maslist.com`, `streamsgalaxy.com`, `yesuinnu.com`; *already fail* CHECK_150 today → convert to redirect), **3 www-SSL-broken** (`lamill.us`, `lamillrentals.com`, `virtually.co.in` — record exists, TLS fails). So "34 apex-only" was loose: it's **33** genuinely-no-www + 3 broken-www, and 11 already do exactly the target.

**Exclusions: parked outliers are out of scope.** The three SSL-broken domains overlap the apex-broken outliers and read as parked/non-launched, not live sites — excluded from the mandate. Two self-exclude under the **apex-must-be-200 scope rule** (`lamill.us` apex-404, `virtually.co.in` apex-SSL-broken — no live apex to redirect *to*); `lamillrentals.com` (apex 200, www broken) is excluded explicitly via the existing `[fleet] dark_sites`/archived seam that `fleet fix` already honors. **v46 adds no new exclusion list** — it reuses the fixer's existing skip filters.

**Why this is real build work, not a one-line severity flip.** The current CF fixer (`_apply_cf_always_use_https`) only toggles `always_use_https` — **zero** www capability — and `cloudflare.py` has DNS create/delete + custom-domain attach but **no redirect-rule mechanism at all**. The genuinely new thing is the Rulesets-API redirect helper (v46.B); everything else composes existing seams (`create_dns_record`, the fixer dispatch, `_deploy_cf_unified`). The 11 already-conformant zones de-risk this: v46.B mirrors a live working rule instead of guessing the API shape.
