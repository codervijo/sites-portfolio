...
| v35.B | ⏳ | **Shared HTTP layer + transient/permanent taxonomy (H4 + H7).** New `_httpapi.py` (`managed_client()` ctx + `raise_for` + `HttpApiError`/`TransientHTTPError`/`PermanentHTTPError` + canonical `RETRYABLE_STATUSES`) so the `↷`/`✗` color-code rule is enforceable fleet-wide. **ADR-0024.** Providers keep typed error classes, reparented onto the taxonomy. **Done:** foundation + 26 tests; migrated `godaddy`, `vercel`; `serp` sources the retryable set. Full suite green (3234). **Remaining:** migrate `cloudflare` (deploy-critical — own increment), `ga4_admin`, `gsc_admin` (+ opportunistically `gtrends`/`indexnow`/`porkbun_dns`). |
