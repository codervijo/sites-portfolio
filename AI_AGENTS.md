...
- `data/portfolio.json` — canonical inventory + classifications, generated by `portfolio info cleanup`. Each domain has `name, registrar, category, expires, ...`. Drives `info category`, `info wip`, and `info status` resolution.
