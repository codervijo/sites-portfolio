...
**What it is.** A Python+uv CLI (`portfolio`) that manages a personal domain portfolio plus a sibling `sites/<domain>/` workspace — and, since v45, agency-client sites alongside the operator's own. Single user (Vijo), CLI-only, no public surface. (Clients aren't users: a client owns a domain and receives a site; they never touch the CLI.)
...
**Scope today.**
  - 54 operator-owned domains across 3 registrars (GoDaddy 44, Namecheap 7, Porkbun 4), plus any agency-client domains listed in `data/domains/clients.csv`
  - 34 sibling projects under `sites/`
  - Goal: 30 commercial sites
  - Read sources: registrar CSV exports, `data/domains/clients.csv` (client roster, v45), `data/portfolio.json` (canonical), Google Search Console (28-day window), CrUX (where it has data), HTTP live-probe, git logs, project conformance scan
...
**Five powers.**
  ...
  4. *Domain acquisition* — `new domain <topic>`: OpenAI brainstorm + RDAP availability + Porkbun pricing → interactive shortlist → register.
...
**Two populations, one fleet (v45).** lamill manages the operator's own sites *and* agency-client sites, told apart by the `owner` field on each domain. `owner = "lamill"` (the default, and everything pre-v45) means the operator holds the domain; any other value names an agency client who owns **only** the domain — the operator builds, deploys, and hosts the site and runs Cloudflare, GSC, GA4, and GitHub under their own accounts. A client site is otherwise an ordinary fleet member: every check, fixer, deploy step, and SEO probe applies to it unchanged. A missing or blank `owner` always reads as `"lamill"`, so nothing can fail on its absence.
...
- `lamill new bootstrap <domain>` — scaffold a sites/* project to ship-ready (v3.A; Power 2). Astro+Vite is the only supported stack (ADR-0013). `--git-url <url>` clones a Lovable-style export into `genai/` then copies up; non-Astro repos are auto-translated via Claude subprocess (v15.H). `--from-genai` (existing repo with `genai/` subdir) follows the same translation hook. Interactive prompts collect AI_AGENTS sections + domain inventory + growth hypothesis; smart-paste accepts a multi-section LLM response and auto-fills (v15.G era). Default platform = `cf-workers` (was cf-pages pre-v15.I per ADR-0012).
- `lamill new deploy <domain>` — git-integrated CF Pages-API pipeline (ADR-0012); creates GH repo + CF zone + CF Pages project + custom-domain attach in one idempotent flow. No `wrangler deploy`; commits to `main` auto-deploy. `--yes` auto-confirms the registrar NS update. Vercel/HostGator paths unchanged.
- `lamill new validate <topic> [--verify] [--audit-model gpt-4o] [--no-verify] [--invalidate {none,interpretive,audit,all}]` — multi-keyword SerpAPI cluster + mechanical gates (v8.D) + primary interpretive pass (Claude CLI, v8.I) + adversarial audit pass (OpenAI; v12.A-C; opt-in via `--verify`) + reconciliation surfacing `REVIEW_REQUIRED` when the two models disagree (v12.D-E). `verify_by_default = true` in `sites/portfolio/lamill.toml [operator]` flips audit-on; `--no-verify` opts out per-run (v12.F).
...
- `data/domains/clients.csv` — v45.D, the fourth sync source. Operator-authored roster of agency-client domains (`domain,owner,category`); hand-edited, or appended by `new bootstrap --owner "<client>"`. Client domains appear in no registrar export, and `fleet sync` deletes any row not backed by a source — so the roster *is* their source. Everything else about them stays derived (registrar `other`, RDAP dates, first-commit launch inference).
- `data/portfolio.json` — canonical inventory + classifications, generated by `portfolio fleet sync` (v7.A; was `portfolio info cleanup`). Each domain has `name, registrar, category, expires, ...`. Drives `fleet domains --summary --verbose` and `project check` resolution.
