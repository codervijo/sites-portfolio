...
- `data/portfolio.json` — canonical inventory + classifications, generated by `portfolio info cleanup`. Each domain has `name, registrar, category, expires, ...`. Drives `info list --grouped` (the merged successor of `info category`/`info wip` from v5.F.1) and `info status` resolution.
